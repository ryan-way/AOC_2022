...
impl Instruction {
    ...
    pub fn run(&self, ship: &mut Ship) {
        let len = ship.stacks[(self.dest-1) as usize].containers.len();
        for _ in 0..self.count {
            ...
            dest_stack.containers.insert(len, container);
        }
    }
}
...
fn parse_input(lines: &Vec<String>) -> (Ship, Vec<Instruction>){
    let mut ship = Ship {
        stacks: vec![],
    };
...
fn main () {
    let lines: Vec<String> = io::stdin().lines()
        .map(|x| x.unwrap())
        .collect();
    let (ship, instructions) = parse_input(&lines);
